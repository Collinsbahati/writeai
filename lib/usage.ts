...
export async function checkUsage(userId: string): Promise<{ allowed: boolean; count: number; limit: number }> {
  ...
  const { data: usage } = await supabase.from('usage').select('count').eq('user_id', userId).eq('date', today).single()
...
export async function incrementUsage(userId: string) {
  const supabase = getSupabase()
  const today = new Date().toISOString().split('T')[0]
  const { data: existing } = await supabase.from('usage').select('id, count').eq('user_id', userId).eq('date', today).single()
  if (existing) {
